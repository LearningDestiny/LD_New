...
export default function ManageWorkshops() {
  ...
  const handleDeleteMonth = (monthIndex) => {
    let updateCorrectMonth = editingWorkshop.roadmap.filter((month, index) => index
      !== monthIndex);
    setEditingWorkshop((prev) => {
      if (!prev) return null;
      return { ...prev, roadmap: updateCorrectMonth };
  ...
  const handleDeleteWeek =(monthIndex,weekIndex)=>{
    let updateCorrectMonth = editingWorkshop.roadmap.map((month, index) => {
      if (index === monthIndex) {
        let updateCorrectWeek = month.weeks.filter((week, index) => index !== weekIndex);
        return { ...month, weeks: updateCorrectWeek };
      }
      return month;
    }
  ...
  const handleDeleteWeekTopic = (monthIndex,weekIndex,topicIndex)=>{
    let updateCorrectMonth = editingWorkshop.roadmap.map((month, index) => {
      if (index === monthIndex) {
        let updateCorrectWeek = month.weeks.map((week, index) => {
          if (index === weekIndex) {
            let updateCorrectTopic = week.topics.filter((topic, index) => index !== topicIndex);
                      ...
                      >
                        <div className="flex items-start w-full ">
                          ...
                          >
                            <FaTrash
                              className=" text-xs cursor-pointer"
                              onClick={() => handleDeleteWeek(monthIndex,weekIndex)}
                            />
                          </div>
                            ...
                            {week.topics.map((topic, topicIndex) => (
                              <div className="flex items-start w-full">
                                ...
                                >
                                  <FaTrash
                                    className=" text-xs cursor-pointer"
                                    onClick={() => handleDeleteWeekTopic(monthIndex,weekIndex,topicIndex)}
                                  />
                                </div>
